...
const GovernanceImports = ['GovV3Helpers', 'IPayloadsControllerCore', 'PayloadsControllerUtils'] as const;
...
function generateEngineImport(code: string) {
  ...
  if (matches.length > 0) return `import {${matches[0]}} from 'aave-helpers/${matches[1].toLowerCase()}-config-engine/${matches[0]}.sol';\n`;
}
