...
export async function percentPrompt<T extends boolean>({ message, required }: GenericPrompt<T>, opts?): Promise<string> {
...
export function translateJsPercentToSol(value?: string, bpsToRay?: boolean) {
  ...
  const _value = (Number(value) >= 1 ? formattedValue : formattedValue.replace(/^0\.0*(?=[0-9])/, '')).replace(/[\.,]/g, '_');
