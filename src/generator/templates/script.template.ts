import { CHAIN_TO_CHAIN_ID, generateContractName, generateFolderName, getChainAlias } from '../common';
...
//     .join(', ')}} from 'aave-helpers/ScriptUtils.sol';\n`;
  /*
    template += options.features
      .map((feature) => {
        const name = generateContractName(options, feature);
  ...
  /*
    template += Object.keys(poolsToChainsMap)
      .map((chain) => {
        return `/!**
        * @dev Deploy ${chain}
        * deploy-command: make deploy-ledger contract=src/${folderName}/${fileName}.s.sol:Deploy${chain} chain=${getChainAlias(chain)}
        * verify-command: npx catapulta-verify -b broadcast/${fileName}.s.sol/${CHAIN_TO_CHAIN_ID[chain]}/run-latest.json
        *!/
       contract Deploy${chain} is ${chain}Script {
         function run() external broadcast {
           // deploy payloads
           ${poolsToChainsMap[chain]
             .map(({ contractName, pool }, ix) => `address payload${ix} = GovV3Helpers.deployDeterministic(type(${contractName}).creationCode);`)
             .join('\n')}

           // compose action
           IPayloadsControllerCore.ExecutionAction[] memory actions = new IPayloadsControllerCore.ExecutionAction[](${poolsToChainsMap[chain].length});
           ${poolsToChainsMap[chain].map(({ contractName, pool }, ix) => `actions[${ix}] = GovV3Helpers.buildAction(payload${ix});`).join('\n')}
  ...
  /*
    ...
    contract CreateProposal is EthereumScript {
      function run() external {
        // create payloads
        PayloadsControllerUtils.Payload[] memory payloads = new PayloadsControllerUtils.Payload[](${Object.keys(poolsToChainsMap).length});
        ...
        // create proposal
        vm.startBroadcast();
        GovV3Helpers.createProposal(vm, payloads, GovV3Helpers.ipfsHashFile(vm, 'src/${folderName}/${options.shortName}.md'));
      }
    }`;
